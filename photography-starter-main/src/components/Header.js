...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (offset > 50) { // Adjust this value based on when you want the effect to apply
        setIsScrolled(true);
      } else {
        setIsScrolled(false);
      }
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full px-[30px] lg:px-[100px] z-30 h-[100px] lg:h-[80px] flex items-center transition-all duration-300 ease-in-out ${
        isScrolled ? 'backdrop-blur-md bg-white/70 shadow-lg' : 'bg-transparent'
      }`}
    >
